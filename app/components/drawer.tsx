...
export function Drawer({
  ...
  const onPointerUp = () => {
    ...
    if (
      drag.y > height * SWIPE_FRACTION ||
      (drag.y > 8 && velocity > SWIPE_VELOCITY)
    ) {
      onOpenChange(false);
    }
    setDrag(null);
  ...
  const height = panelRef.current?.offsetHeight ?? 1;
  const backdropOpacity = shown
    ? Math.min(1, Math.max(0, 1 - dragY / height))
    : 0;
  const settleMs = shown ? ENTER_MS : EXIT_MS;
  const transition =
    dragging || reduceMotion
      ? "none"
      : `transform ${settleMs}ms ${EASE_DRAWER}`;
  ...
  const backdropStyle: CSSProperties = {
    ...
    transition:
      dragging || reduceMotion
        ? "none"
        : `opacity ${settleMs}ms ${EASE_DRAWER}`,
  };
  ...
  return (
    <dialog
      ...
      className="text-foreground fixed inset-0 m-0 h-dvh max-h-none w-screen max-w-none items-end justify-center overflow-hidden bg-transparent p-0 backdrop:bg-transparent open:flex"
    >
      <button
        type="button"
        aria-label="Close"
      ...
      <div
        ...
        style={panelStyle}
        className={cn(
          "bg-background relative flex w-full max-w-lg flex-col overflow-y-auto rounded-t-3xl pb-[env(safe-area-inset-bottom)] shadow-[0_0_0_1px_var(--border-strong),0_-24px_60px_-20px_rgb(0_0_0/0.45)]",
          "max-h-[85dvh]",
          className,
        )}
      >
        ...
        >
          <span
            aria-hidden="true"
            className="bg-foreground/20 mx-auto h-1.5 w-10 shrink-0 cursor-grab rounded-full active:cursor-grabbing"
          />
          <div className="flex items-start justify-between gap-4 pt-3 pb-4">
            ...
            <button
              ...
              aria-label="Close"
              onClick={() => onOpenChange(false)}
              className="text-muted-foreground hover:bg-muted hover:text-foreground relative -mt-1 -mr-2 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-colors duration-150 after:absolute after:-inset-1.5"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        {footer ? (
          <div className="flex flex-wrap justify-end gap-2 px-6 pt-6 pb-6">
            {footer}
          </div>
        ) : null}
      </div>
